...
(
    ...
    `sample` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否样品 0否 1是；样品计入销售额但不计利润',
    `profit_cost_amount` decimal(18, 4) DEFAULT NULL COMMENT '利润核算成本（历史核定整单成本）',
...
(
    ...
    `base_price`      decimal(18, 4)           DEFAULT NULL COMMENT '基础销售单价快照（不含属性加价）',
    `attribute_extra_amount` decimal(18, 4)    DEFAULT NULL COMMENT '单件属性加价快照',
    `cost_price`      decimal(18, 4)           DEFAULT NULL COMMENT '单件成本价快照',
