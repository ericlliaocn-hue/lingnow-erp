<template>
  <div class="app-container stock-balance-page">
    ...
    <el-card shadow="never">
      <template #header><div class="card-header"><strong>库存查询</strong><el-button :icon="Refresh" :loading="loading" @click="getList">刷新</el-button></div></template>
      <el-table v-loading="loading" :data="list" border empty-text="暂无库存数据">
        <el-table-column prop="productName" label="商品" min-width="180" />
        <el-table-column prop="warehouseName" label="仓库" min-width="140" />
        <el-table-column prop="qty" label="库存数量" align="right" />
        <el-table-column prop="avgCost" label="平均成本" align="right" :formatter="money" />
        <el-table-column prop="costAmount" label="库存金额" align="right" :formatter="money" />
...
const query = reactive({ current: 1, size: 10, productId: undefined as string | undefined, warehouseId: undefined as string | undefined })
function getList() {
  loading.value = true
  stockBalance(query).then(res => { list.value = res.records; total.value = Number(res.total) }).finally(() => loading.value = false)
}
